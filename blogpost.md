...
### Introduction
It is likely that you've seen this old and famous [demo presentation](https://www.youtube.com/watch?v=Gzj723LkRJY) on how powerful and easy it is to build an app with Ruby on Rails, as the demo shows it took only 15 minutes to build a blog.

But what if I told you that Rails can also be used to quickly and easily create an API, using all available tools that the framework provides, same security, same generators and most of all, same agility and easy-to-understand language?

Today we'll dive into Rails API mode and how it can be used to create powerful - yet simple - APIs for your back-end!

### What is an API?
First of all, what _is an API?_, chances are that you've seen two definitions, so let us get this out of the way. API stands for _Application Programming Interface_ and is mostly used to describe an accesible documentation of a framework, programming language or service so a developer can learn, check or revisit any aspect of these tools, as an example we have the [GitHub API](https://docs.github.com/)(also refered and the GitHub Docs) or the [Rails API](https://api.rubyonrails.org/)(once again, also referred as the Rails Docs).

Although we can understand API as a thorough documentation of a tool, we can also find public and privates APIs that are applications that consumes static resources via other applications or HTTP clients calls, it is a way for one system to interact with other external systems, that means that there are application out there that we access differently from the "common way" of interacting with a system with generated HTML that we click and bop, these APIs are usually accessed using direct HTTP calls, usually consuming a JSON resource.

Summarizing, instead of generating a page to be interacted through forms, links and views the application can be communicated with exposed endpoints that provide information in a set static structure.

### Why RoR for APIs?
Rails was created following the [Model-View-Controller (MVC)](https://en.wikipedia.org/wiki/Model%E2%80%93view%E2%80%93controller) principles and it can be used to build both the front-end and back-end of a system, but following this MVC architecture we can use Rails to have its views served as JSON resources, you'll develop focused on your models and controllers and serve the needed information to the outside world in a static pre-constructed structure, so no need to be in charge of creating a front-end or complex view, you'll only need to organize the structure that will be served, the application that will consume what you provide will do as they please with the JSON you created.

This means you can keep the happy-coding you already have for creating full-stack RoR application while creating APIs, focusing mainly on controllers and models. Although, there are simpler frameworks to create APIs using Ruby, RoR is a full-stack framework with a bunch of tools, wouldn't it be better to use something else? Well, _it depends_, as mentioned in the [Rails Guides](https://guides.rubyonrails.org/api_app.html#why-use-rails-for-json-apis-questionmark):
...
> The short version is: you may not have thought about which
...
There are three main differences between creating a common RoR app and RoR app on api mode, as per described in the Rails Guides:

* **API mode has a limited set of middleware:** No middlewares for browser applications
...
The middleware is now _leaner_, removing unnecessary features for API, no cookie support and so forth.

[ActionController::API](https://api.rubyonrails.org/classes/ActionController/API.html) is an lightweight version of [ActionController::Base](https://api.rubyonrails.org/classes/ActionController/Base.html), it allows us to create controllers with just the features an API need, no overly "fancy" functionalities used in a normal Rails controller. In a nutshell it does not have features that a browser requires, like layouts and templates rendering, flash, assets and cookie, but of course, you have the freedom to add them as needed.
...
When rendering or redirecting there are a couple thing to keep in mind, we can freely choose to render, as an example using `render json: 'resource'`, but remember that since there no templates, no implicit rendering will be made, so it is _necessary_ to either call `render` or `redirect_to`

Either way we have some options on how to handle, for rendering we can form an structure similar to how we see them in regular rails, just explicitly telling what to do afterwards
...
There are, of course, more options then just JSON, but that's what you willl usually deal with, but in case you are curious...
...
* Status code matter a bit more, since as an API this info is usually needed
  * ```rb
      redirect_to url, status: :see_other           # 303
      redirect_to url, status: :moved_permanently   # 301
    ```

So we'll need to handle and understand the _intention_ of the response! Always render or redirect, set status code and never assume clients handle redirects like browsers do!

Lastly, before our practice, let's see some file and folder differences when creating a normal and an api RoR project. When creating an API you'll notice right away the it contains less folders
...
```
We can also see some notable differences in the `app` folder
```bash
### Non API mode
app/
├── assets/
...
### RoR API Mode
Let's put our new tool knowledge to practice then! What better way to learn? We'll create a simple app to gather information about our beloved miners, let's call it "PocketMiner42"
...
To create a new resource we do it the same as always, we'll create a Miner with name and level and Rare Gems that belongs to Miner, with name and color.
...
We can do as we please, maybe create a service object to input some decision on how the Miners get theirs Rare Gems, create transactions and contracts for a stronger business/miner logic, just as you would in a normal rails app, the backend will work the same, the main difference here is how the data is passed to the end user.

### JSON, Serializing and JBuilder
Given we will mainly use JSON, how does it look like on an typical GET call on the API endpoint? After creating some Miners and Gems we have:
```
[
	{
		"id": 1,
...
As of Rails 8 it should came in your gemfile as a commented line, just uncomment it and do a bundle install.
```rb
# gemfile.rb
